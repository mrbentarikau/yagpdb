...
var Command = &commands.YAGCommand{
	...
	RunFunc: func(data *dcmd.Data) (interface{}, error) {
		...
		if data.Switches["id"].Value != nil {
			...
			const q = `SELECT member_count, name, row_number FROM (SELECT id, member_count, name, row_number() OVER (ORDER BY member_count DESC) FROM joined_guilds where left_at IS NULL) AS total WHERE id=$1;`
